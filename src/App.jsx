...
import {
 Plus, Check, Search, Timer, Wind, BookOpen, Archive,
 Palette, Star, LogOut, User, X, Sparkles, KeyRound, ArrowRight, Loader, Heart
} from 'lucide-react';
...
const THEMES = {
 dawn: {
   id: 'dawn',
   name: 'Рассвет',
   bg: 'linear-gradient(135deg, #fff1f2 0%, #ffe4e6 50%, #fff7ed 100%)',
   primary: '#e11d48',
   text: '#881337',
 ...
 noir: {
   id: 'noir',
   name: 'Нуар',
   bg: 'linear-gradient(135deg, #171717 0%, #262626 100%)',
   primary: '#fafafa',
   text: '#a3a3a3',
   card: 'rgba(64, 64, 64, 0.5)'
 }
};
...
const AmenApp = () => {
 ...
 const logout = () => { signOut(auth); setNickname(""); setPassword(""); };
 ...
 const saveAnswer = async () => {
   ...
   // Исправлено: правильное определение коллекции для обновления
   ...
   await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, coll, id), { status: 'answered', answeredAt: serverTimestamp(), answerNote: inputText });
 };
 ...
 return (
   <div className="animated-bg" style={{ minHeight: '100vh', background: cur.bg, fontFamily: '-apple-system, sans-serif', color: cur.text, transition: 'color 0.5s' }}>
     ...
     <div style={{maxWidth: 500, margin: '0 auto', minHeight: '100vh', display: 'flex', flexDirection: 'column'}}>
       ...
       {/* TABS */}
       <div style={{display: 'flex', padding: '0 24px', marginBottom: 10, gap: 10}}>
         {[{id:'home', l:'Дневник'}, {id:'list', l:'Список'}, {id:'vault', l:'Чудеса'}].map(tab => (
       ...
       {/* FAB */}
       <div style={{position: 'fixed', bottom: 30, left: 0, right: 0, display: 'flex', justifyContent: 'center', gap: 24, pointerEvents: 'none', zIndex: 10}}>
         <motion.button whileTap={{scale:0.9}} onClick={() => setModalMode('focus')} style={{pointerEvents: 'auto', width: 56, height: 56, borderRadius: '50%', background: isDark?'#334155':'white', border: 'none', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: isDark?'white':'#94a3b8', cursor: 'pointer'}}><Timer size={24}/></motion.button>
         <motion.button whileTap={{scale:0.9}} onClick={() => { setModalMode(activeTab === 'list' ? 'topic' : 'entry'); setInputText(""); }} style={{pointerEvents: 'auto', width: 72, height: 72, borderRadius: '50%', background: cur.primary, border: 'none', color: isDark?'black':'white', boxShadow: `0 10px 40px ${cur.primary}50`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer'}}><Plus size={36} strokeWidth={2.5}/></motion.button>
       </div>
     </div>
     ...
     {modalMode === 'focus' && (
       <div style={{position: 'fixed', inset: 0, background: isDark?'#0f172a':'white', zIndex: 200, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
         ...
         <h2 style={{fontFamily: 'serif', fontSize: 48, color: cur.text, margin: 0}}>Тишина</h2>
