...
const INITIAL_DATA = [
 ...
 // ... (rest of initial data kept for brevity, structure assumed same)
...
const AmenApp = () => {
    ...
    // --- ONBOARDING STATE ---
    const [onboardingStep, setOnboardingStep] = useState(0); // 0: Mood, 1: Result, 2: Auth
    ...
    useEffect(() => { const unsub = onAuthStateChanged(auth, (u) => { 
        setUser(u); 
        if (u) {
            setLoading(false);
            // If user just registered from onboarding flow, save their mood
            if (selectedMood) {
                // We save it as a prayer automatically
                ...
                saveInitialPrayer();
                setSelectedMood(null); // Clear after saving
            }
        }
        setAuthLoading(false); 
    }); return () => unsub(); }, [selectedMood]);
                ...
                <motion.div initial={{opacity:0}} animate={{opacity:1}} style={{height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding: 20}}>
                    <div style={{background: cur.card, padding: 30, borderRadius: 30, backdropFilter: 'blur(10px)', width: '100%', maxWidth: 320, boxShadow: '0 10px 40px rgba(0,0,0,0.1)'}}>
                        ...
                        <div style={{display: 'flex', flexDirection: 'column', gap: 12}}>
                            ...
                            {selectedMood && <button onClick={() => setOnboardingStep(0)} style={{background:'none', border:'none', fontSize:12, opacity:0.5, marginTop:10, cursor:'pointer'}}>Назад к выбору</button>}
                        </div>
                    </div>
                </motion.div>
            );
            ...
            {modalMode === 'about' && (
                <div style={{position: 'fixed', inset: 0, background: isDark ? 'rgba(15, 23, 42, 0.9)' : 'rgba(255,255,255,0.95)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20}} onClick={closeModal}>
                    <motion.div initial={{scale:0.9, opacity:0}} animate={{scale:1, opacity:1}} style={{background: isDark ? '#1e293b' : 'white', width: '100%', maxWidth: 350, borderRadius: 30, padding: 30}} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{textAlign:'center', fontSize: 11, opacity: 0.4, color: isDark ? 'white' : 'black'}}>Версия 1.5</div>
                    </motion.div>
                </div>
            )}
