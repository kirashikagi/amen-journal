...
const AmenApp = () => {
    ...
    const handleReflection = async () => {
        ...
        if (window.confetti) window.confetti({ shapes: ['star'], colors: ['#FFD700', '#FFA500'] });
    };

    const handleFocusPray = async () => {
       await updateStreak();
       setDailyFocusDone(true);
       if (window.confetti) window.confetti({ particleCount: 150, spread: 100, origin: { y: 0.6 }, colors: [cur.primary, '#fbbf24', '#ffffff'] });
    };
            ...
            {onboardingStep === 0 ? (
                <>
                    ...
                    <div style={{display:'flex', flexDirection:'column', gap:10}}>
                        {[{id:'anxiety',label:'Тревога',icon:<Wind size={24}/>}, {id:'weary',label:'Усталость',icon:<Coffee size={24}/>}, {id:'joy',label:'Радость',icon:<Sun size={24}/>}].map(o => (
                                ...
                                {modalMode === 'scripture_finder' ? renderScriptureFinder() :
                                 modalMode === 'about' ? (
                                    <motion.div initial={{scale:0.9, opacity:0}} animate={{scale:1, opacity:1}} onClick={e => e.stopPropagation()} style={{background: cur.card, padding: 30, borderRadius: 30, maxWidth: 350, maxHeight: '80vh', overflowY: 'auto', backdropFilter:'blur(10px)'}}>
                                        ...
                                        <div style={{marginTop:20, fontSize:11, opacity:0.5, textAlign:'center'}}>Версия 5.3</div>
                                    </motion.div>
                                ) : (
